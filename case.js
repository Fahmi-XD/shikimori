...
const pathMenu = './database/menuData.json';
const { initializeUser, initializeChat, initializeSetting } = require('./lib/db');
...
module.exports = ditz = async (ditz, m, msg, chatUpdate, store) => {
  try {
    ...
    function replyBanned() {
      ditz.sendMessage(m.chat, { text: mess.banned, footer: 'ShikimoriBotz - By DitzDev', buttons: [{
        buttonId: '.banding',
        buttonText: {
    ...
    if(m.mtype === "interactiveResponseMessage"){  
            ...
            if(msg.nativeFlowResponseMessage  && !m.isBot  ){ 
                ...
                if(id){
                    ...
                    return ditz.ev.emit("messages.upsert" , { messages : [ emit_msg ] ,  type : "notify"})
                }
            }
        }
    // ============== END OF BOT SYSTEM ===========
    ...
    function loadMenuData() {
      if (fs.existsSync(pathMenu)) {
        const data = fs.readFileSync(pathMenu, 'utf8');
    ...
    if (!global.menuData) {
      global.menuData = loadMenuData();
    }
    
    function addCommandToCategory(category, cmd, desc) {
...
case 'ai': case 'shikimori': {
   addCommandToCategory("AI", "AI", "Berbicara dengan AI, Tanya apapun yang kamu suka!");
...
break;
case 'claude': {
  addCommandToCategory("AI", "Claude", "Tanya apa saja dengan AI dari Anthropic!");
  if (isBan) return replyBanned();
...
//////// DOWNLOADER HERE /////////////////////
case 'tiktok': case 'tt': {
  addCommandToCategory("Downloader", "TikTok", "Download video TikTok Tanpa Watermark");
  if (isBan) return replyBanned();
...
case "youtube": case "ytdl": {
   addCommandToCategory("Downloader", "YouTube", "Download Video/Audio YouTube dengan instant.");
   reply("Worek");
}
break;
      default:
